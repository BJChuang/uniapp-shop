					...
					<block v-for="(item, index) in treeList" :key="index">
						<view class="tree-item" :style="[{
							...
							<view class="item-label">
								...
								<view class="uni-flex-item uni-inline-item" style="display: flex;font-size: 26rpx;align-items: center;" @tap.stop="_onItemSelect(item, index)">
		...
		methods: {
			...
			_confirm() { //多选
				...
				this.treeList.forEach((item, index) => {
					...
					// }
					if (item.checkStatus === 2) {
						if(item.level == 0) firstList.push(item)
						if(item.level == 1) secList.push(item)
						if(item.level == 2) thrList.push(item)
						selectedList.push(item.id);
						selectedNames = selectedNames ? selectedNames + ' / ' + item.name : item.name;
					}
				})
				...
				if(thrList.length > 0) {
					...
					for(let i=0;i<thrList.length;i++){
						let titem = [0,thrList[i]['parentId'],thrList[i].id]
						for(let s=0;s<secList.length;s++){
							if(secList[s].id == thrList[i]['parentId']){
								titem[0] = secList[s]['parentId']
							}
						}
						realList.push(titem)
					}
				...
				this._hide()
				this.$emit("select-change", realList, selectedNames);
			},
			//格式化原数据（原数据为tree结构）
